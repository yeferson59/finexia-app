...
| 7 | Fase 4 (PR A) | Duplicaciones temporales por la convivencia legacy↔módulo: `services/token_helpers.go` (token+truncate/sanitizeIP, muere en PR C), `auth/postgres.go::createUser` (copia de `repositories/user.go::CreateUser`, se unifica en Fase 5), `sendPasswordChangedAlert`+helpers en `services/user.go` vs. alertas del módulo (Fase 5), `nextRunTime` copiado en `auth/cleanup_job.go` (Fase 7). | Cada copia tiene dueño y fase de eliminación; verificar su borrado al cerrar esas fases. |
