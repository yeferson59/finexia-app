 ...
 * sidebar) y los bloques de la portada del panel, más los dos que reutiliza
 * el detalle de portafolio: `PortfolioGrowth` y `PeriodReturns` (con
 * `hasTrailingReturns`, para no envolver una franja que no se pinta). Lo
 * interno de un bloque —la curva en miniatura, el detalle de la gráfica— no
 * sale de aquí; el selector de moneda dejó de ser interno y vive en `lib/ui`,
 * porque la vista de activos lo comparte.
 */
...
export { default as PeriodReturns } from './components/period-returns.svelte';
export { hasTrailingReturns } from './trailing';
