<script lang="ts">
	...
	const validatePassword = (password: string, minLength = 8): string | null => {
	...
	const handleRegisterSubmit = async (e: SubmitEvent) => {
		...
		if (!agreeTerms) {
			registerErrors['terms'] = 'You must agree to the terms and conditions';
		}
...
<main class="auth-container">
	...
	<div class="auth-card">
		...
		<section class="forms-container">
			<!-- Login Form -->
			{#if isLoginMode}
				<form onsubmit={handleLoginSubmit} class="form-content" id="login-form">
					<Input
						label="Email"
						id="login-email"
						name="email"
					...
					<div class="form-switch">
						¿No tienes cuenta?
						<button type="button" onclick={toggleMode} class="switch-link">
							Crear una
						</button>
					</div>
				</form>
