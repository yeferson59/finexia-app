		...
		{#each activities as activity (activity.id)}
			<article class={`activity-item activity-${activity.type}`}>
				<div class={`activity-icon ${activity.type}`}>
					...
					{:else}
						<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
							<path d="M16 8v-2c0-.55-.45-1-1-1H9c-.55 0-1 .45-1 1v2H6v2h2v7c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-7h2V8h-2zm-3 8h-4v-7h4v7z"></path>
						</svg>
					{/if}
				</div>
				...
				<div class="activity-details">
					<p class="activity-amount" class:positive={['deposit', 'dividend'].includes(activity.type)}>
						{['deposit', 'dividend'].includes(activity.type) ? '+' : '-'}${new Intl.NumberFormat('es-CO').format(activity.amount)}
					</p>
					<time class="activity-date" datetime={`${activity.date}T${activity.time}`}>{formatDate(activity.date)}</time>
				</div>
			</article>
		{/each}
	</div>
