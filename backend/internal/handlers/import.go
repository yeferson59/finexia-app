...
func parseImportForms(c fiber.Ctx) (*portfolio.ImportMappingDTO, portfolio.ImportDefaultsDTO, error) {
	var mapping *portfolio.ImportMappingDTO
	if raw := c.FormValue("mapping"); raw != "" {
		mapping = &portfolio.ImportMappingDTO{}
		if err := json.Unmarshal([]byte(raw), mapping); err != nil {
			return nil, portfolio.ImportDefaultsDTO{}, errors.New("invalid mapping: malformed JSON")
		}
	}

	var defaults portfolio.ImportDefaultsDTO
	if raw := c.FormValue("defaults"); raw != "" {
		if err := json.Unmarshal([]byte(raw), &defaults); err != nil {
			return nil, portfolio.ImportDefaultsDTO{}, errors.New("invalid defaults: malformed JSON")
		}
	}
...
// anything.
func (h *Handlers) PreviewTransactionsImport(c fiber.Ctx) error {
	...
	preview, err := h.services.PreviewTransactionImport(data, filename, c.FormValue("sheet"), mapping, defaults)
	if err != nil {
		return h.responseFromDomain(c, err, "Error parsing spreadsheet", "Could not parse the uploaded file")
	}

	return h.responseStatusOk(c, "Import preview generated", "Spreadsheet parsed successfully", preview)
}
...
func (h *Handlers) ImportTransactions(c fiber.Ctx) error {
	...
	if err != nil {
		return h.responseBadRequest(c, "Invalid portfolio ID", "portfolioId must be a valid UUID")
	}
	sourceID, err := uuid.Parse(c.FormValue("sourceId"))
	if err != nil {
		return h.responseBadRequest(c, "Invalid source ID", "sourceId must be a valid UUID")
	}
	...
	return h.responseStatusOk(c, "Transactions imported", "Spreadsheet imported successfully", result)
}
