...
func TestSyncExchangeRates(t *testing.T) {
	...
	t.Run("unparseable rates are rejected", func(t *testing.T) {
		...
		repo := &fakeRepository{
			upsertExchangeRate: func(context.Context, string, string, money.Decimal, time.Time) (ExchangeRate, error) {
	...
	t.Run("a successful pair is persisted and cancellation stops the run", func(t *testing.T) {
		...
		repo := &fakeRepository{
			upsertExchangeRate: func(_ context.Context, from, to string, rate money.Decimal, rateDate time.Time) (ExchangeRate, error) {
				...
				cancel()
				return ExchangeRate{FromCurrency: from, ToCurrency: to, Rate: rate, RateDate: rateDate}, nil
			},
		}
		storage := newMemStorage()
