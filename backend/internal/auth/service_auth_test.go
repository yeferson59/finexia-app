...
func verifiedUser(t *testing.T, password string) identity.User {
	t.Helper()
	return identity.User{
		...
		Accounts:      []identity.Account{{Password: mustHashPassword(t, password)}},
	}
}
...
func TestValidateToken(t *testing.T) {
	...
	t.Run("valid token passes and is cached", func(t *testing.T) {
		...
		sessionUser.Sessions = []identity.Session{{Token: token}}
	...
	t.Run("cached negative result is rejected", func(t *testing.T) {
		svc, storage, _ := newSvc(identity.User{}, errors.New("must not be called"))
	...
	t.Run("token without matching session is rejected", func(t *testing.T) {
		svc, _, _ := newSvc(identity.User{}, errors.New("no session"))
		token := signToken(t, svc, time.Now().UTC().Add(10*time.Minute))
	...
	t.Run("session token mismatch is rejected", func(t *testing.T) {
		sessionUser := user
		sessionUser.Sessions = []identity.Session{{Token: "a-different-token"}}
		svc, _, _ := newSvc(sessionUser, nil)
		token := signToken(t, svc, time.Now().UTC().Add(10*time.Minute))
...
func (f *refreshFixture) repo() *fakeRepository {
	return f.svc.stores.Accounts.(*fakeRepository)
}
...
func TestRefreshTokenReuseDetection(t *testing.T) {
	t.Run("reuse outside grace period revokes the family", func(t *testing.T) {
		...
		f.repo().getRefreshTokenByHash = func(context.Context, string) (identity.RefreshToken, error) {
...
func TestCleanupExpiredAuth(t *testing.T) {
	t.Run("returns every counter", func(t *testing.T) {
		repo := new(fakeRepository{
