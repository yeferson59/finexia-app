...
func (s *Services) ImportExchangeRatesFromFile(ctx context.Context, data []byte, filename, sheet string) (portfoliodto.ImportResultResponseDTO, error) {
	...
	if len(dataRows) > maxExchangeRateImportRows {
		return portfoliodto.ImportResultResponseDTO{}, fmt.Errorf("invalid spreadsheet: too many rows (max %d)", maxExchangeRateImportRows)
	}
	...
	for i, row := range dataRows {
		...
		if _, err := s.repos.UpsertExchangeRate(ctx, from, to, rate, now); err != nil {
			result.Skipped++
			if len(result.Errors) < 100 {
