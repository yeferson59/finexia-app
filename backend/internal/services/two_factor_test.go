...
func wireTwoFactorRepo(repo *fakeRepository, state *twoFactorState) {
	repo.getTwoFactor = func(_ context.Context, _ uuid.UUID) (entities.TwoFactor, error) {
		if state.row == nil {
	...
	repo.upsertTwoFactorSecret = func(_ context.Context, userID uuid.UUID, secret string) error {
		...
		state.row = &entities.TwoFactor{UserID: userID, Secret: secret}
		return nil
	}
...
func TestTwoFactorSetupAndEnableFlow(t *testing.T) {
	...
	repo := &fakeRepository{
		getAccountByUserID: func(_ context.Context, _ uuid.UUID) (entities.Account, error) {
			return user.Accounts[0], nil
		},
		getUserByID: func(_ context.Context, _ uuid.UUID) (entities.User, error) {
			return user, nil
		},
	}
...
func loginCapableRepo(t *testing.T, user entities.User, state *twoFactorState) *fakeRepository {
	t.Helper()
	repo := &fakeRepository{
		...
		getUserByID: func(_ context.Context, id uuid.UUID) (entities.User, error) {
			if id != user.ID {
				return entities.User{}, errors.New("not found")
			}
			return user, nil
...
func enabledTwoFactorState(t *testing.T, userID uuid.UUID) (*twoFactorState, string) {
	...
	return &twoFactorState{
		row:   &entities.TwoFactor{UserID: userID, Secret: secret, Enabled: true, ConfirmedAt: &now},
...
func TestLoginWithPendingSetupDoesNotRequireCode(t *testing.T) {
	...
	state := &twoFactorState{
		row:   &entities.TwoFactor{UserID: user.ID, Secret: secret},
		codes: map[string]bool{},
	}
	repo := loginCapableRepo(t, user, state)
	svc := newTestServices(repo, newMemStorage())
