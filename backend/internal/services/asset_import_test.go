...
func TestImportAssetsFromFile(t *testing.T) {
	t.Run("valid rows are upserted, invalid rows are skipped and reported", func(t *testing.T) {
		...
		repo := &fakeRepository{
			upsertAsset: func(_ context.Context, ticker, name string, assetType entities.AssetType, exchange, currency string) (entities.Asset, error) {
				...
				return entities.Asset{Ticker: ticker, Name: name, AssetType: assetType, Exchange: exchange, Currency: currency}, nil
			},
		}
	...
	t.Run("repository failures are reported per row without stopping the import", func(t *testing.T) {
		...
		repo := &fakeRepository{
			upsertAsset: func(context.Context, string, string, entities.AssetType, string, string) (entities.Asset, error) {
