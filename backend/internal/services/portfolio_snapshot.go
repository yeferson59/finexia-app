...
func (s *Services) WasPortfolioSnapshotCreatedToday() bool {
	v, err := s.storage.Get(snapshotSyncCacheKey)
	return err == nil && len(v) > 0
}

func (s *Services) SyncPortfolioSnapshots(ctx context.Context) (int, []error) {
	log := s.log.With(logger.Str("job", "portfolio_snapshot_sync"))

	rows, err := s.repos.GetAllPortfolioSummaryRows(ctx)
	if err != nil {
		return 0, []error{err}
	}
	...
	for _, row := range rows {
		if err := s.repos.UpsertPortfolioSnapshot(
			ctx,
			row.PortfolioID,
			today,
