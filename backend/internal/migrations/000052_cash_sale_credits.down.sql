-- Restore the seven functions as 000050 left them, and the link as it was
-- there. The sales' credits stay, as 000050's down migration leaves its own:
-- with no rule giving cash_sale a meaning, each reads as a row that moves no
-- quantity and no flow, and loses its link and its cost first so the restored
-- constraint — only a cash_dividend has a link — holds. The balances the up
-- migration repriced to one keep that price, which is the right one either way.
...
-- Unlink the sales' credits. The write re-runs the restored trigger over their
-- balances, so the quantity stops counting them; it moves neither the quantity
-- nor the type, so no retirement fires with it.
UPDATE transactions SET credited_from = NULL, cost_basis = NULL WHERE type = 'cash_sale';

ALTER TABLE transactions DROP COLUMN IF EXISTS cost_basis;

ALTER INDEX uk_transactions_credited_from RENAME TO uk_transactions_dividend_credit;
ALTER TABLE transactions RENAME CONSTRAINT transactions_credited_from_fkey TO transactions_dividend_id_fkey;
ALTER TABLE transactions RENAME COLUMN credited_from TO dividend_id;
