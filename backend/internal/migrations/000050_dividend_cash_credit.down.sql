-- Restore the seven functions as 000041, 000042 and 000045 left them, and drop
-- the link. The credits stay, as 000041's down migration leaves cash_interest
-- rows: with no rule giving cash_dividend a meaning, each reads as a row that
-- moves no quantity and no flow.
...
-- Re-run the restored trigger over every balance that held a credit, so its
...
UPDATE transactions SET notes = notes WHERE type = 'cash_dividend';

ALTER TABLE transactions DROP CONSTRAINT IF EXISTS chk_transactions_dividend_credit;
DROP INDEX IF EXISTS uk_transactions_dividend_credit;
ALTER TABLE transactions DROP COLUMN IF EXISTS dividend_id;
